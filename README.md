...
| 비교 지표 | 레거시 방화벽 (iptables, Netfilter) | 1세대 AI 방화벽 (PyTorch 기반) | 홈오스타시스 인그레스 방화벽 (본 프로젝트) |
| :--- | :--- | :--- | :--- |
| **핵심 스위칭 로직** | CPU JMP 조건문<br>(분기 예측 실패 위험) | 무거운 행렬 연산 그래프 기반 추론 | **정수 2의 보수 비트 MUX (1클록)** |
| **버스트 전력/메모리 고갈** | Conntrack 테이블 폭주<br>(커널 패닉) | 미분 노드 추적으로<br>VRAM OOM 발생 | **역전파 거세 (`autograd_free`), 공간 복잡도 O(1) 통결** |
| **미지 (Unknown) 공격 탐지** | 불가능<br>(시그니처 매칭 방식의 한계) | 조건부 가능<br>(오탐 및 판정선 오염 위험) | **공분산 행렬식 (Det) 기반 위상 붕괴 감지 (100% 탐지)** |
| **가속기 하드웨어 병목** | 가속기 연동 불가 | 뱅크 충돌 및 무거운<br>텐서 복사 오버헤드 | **SRAM 뱅크 정렬 (Stride 129), Triton 벡터라이징 로드** |
| **차단 규칙 피드백 지연** | 동기식 시스템 콜 룰셋 갱신 (초 단위) | 통제 플레인 스레드 스탈 발생 | **락프리 링버퍼 덤프 및 커널 HBM 맵 직접 하이재킹 (ms 단위)** |
